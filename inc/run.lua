...
echo -e "${GREEN}
    ...
    ▄▇                                           ¦ Dev : @A_j_o_k_e_r
 ▄▇ ¦ Dev : @A_j_o_k_e_r                    
                                                               ¦ Source SELA BY @V_P_E
----------------------------------------------------------------------${CNIL}"
echo -e "${RED}| للاسف لايمكنك تنصيب السورس ع نظام اوبنتو {${Version}} يجب ان يكون على اصدار  اقل مثلا  ${SMAY}14 او 16
${CNIL}";
echo -e "${RED}| Sorry Canot install Source on Ubuntu {${Version}} Please install system To Ubuntu ${SMAY}14 or 16 ${RED} For Run The Source SELA 
${CNIL}";
exit;
fi
...
¦ NOW I Run File ${Yellow}./run${WHITB} To install ${SMAY}Source SELA${CNIL} . \n
\n\n"
chmod +x ./ins
./ins
exit;
...
if ! screen -list | grep -q "SELA"; then
screen -S SELA ./run R
fi
